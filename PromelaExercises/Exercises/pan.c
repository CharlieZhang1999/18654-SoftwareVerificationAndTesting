...
short src_ln6 [] = {
	  0,  33,  33,  34,  34,  34,  32,  36, 
	 32,  39,  39,  40,  40,  40,  38,  42, 
	 38,  43,  44,   0, };
S_F_MAP src_file6 [] = {
	{ "-", 0, 0 },
	{ "_spin_nvr.tmp", 1, 18 },
...
short src_ln2 [] = {
	  0,  43,  44,  45,  46,  47,  42,  49, 
	 42,  49,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "finalpractice.pml", 1, 9 },
	{ "-", 10, 11 }
};
uchar reached2 [] = {
	  0,   1,   0,   1,   0,   0,   0,   1, 
...
short src_ln1 [] = {
	  0,  32,  33,  34,  35,  36,  37,  37, 
	 31,  39,  31,  39,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "finalpractice.pml", 1, 11 },
	{ "-", 12, 13 }
};
uchar reached1 [] = {
	  0,   1,   0,   0,   1,   0,   1,   1, 
...
short src_ln0 [] = {
	  0,  19,  20,  21,  22,  23,  25,  25, 
	 25,  18,  27,  18,  27,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "finalpractice.pml", 1, 12 },
	{ "-", 13, 14 }
};
uchar reached0 [] = {
	  0,   1,   0,   1,   0,   0,   1,   0, 
...
int Btypes[] = {
   ...
   1,	/* initialState_INIT */
   1,	/* safePedestrian */
   1,	/* pedestrianCanCross */
   1,	/* redStrictlyBeforeWalk */
   0	/* :np_: */
};
...
#ifndef NOCLAIM
	case 8:	/* claim select */
		spin_c_typ[0] = 6; /* redStrictlyBeforeWalk */
		((P8 *)pptr(h))->c_cur[0] = 6;
		reached6[6]=1;
		spin_c_typ[1] = 5; /* pedestrianCanCross */
		((P8 *)pptr(h))->c_cur[1] = 5;
		reached5[5]=1;
		spin_c_typ[2] = 4; /* safePedestrian */
		((P8 *)pptr(h))->c_cur[2] = 6;
		reached4[6]=1;
		spin_c_typ[3] = 3; /* initialState_INIT */
		((P8 *)pptr(h))->c_cur[3] = 4;
		reached3[4]=1;
		((P8 *)pptr(h))->_t = 3;
		((P8 *)pptr(h))->_p = 4;
		((P8 *)pptr(h))->_n = 3; /* initialState_INIT */
		src_claim = src_ln3;
#ifndef BFS
		if (whichclaim == -1 && claimname == NULL)
			printf("pan: ltl formula initialState_INIT\n");
#endif
		if (whichclaim != -1)
...
{
		now.push = 0;
		now.plchan = addqueue(calling_pid, 1, 0);
		now.vlchan = addqueue(calling_pid, 2, 0);
		now.pedchan = addqueue(calling_pid, 3, 0);
#ifdef VAR_RANGES
		logval("push", now.push);
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.plchan) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.plchan) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.plchan) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.plchan) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.vlchan) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.vlchan) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.vlchan) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.vlchan) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.pedchan) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.pedchan) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.pedchan) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.pedchan) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  RB:	5\n");
	printf("	mtype  GREENLIGHT:	4\n");
	printf("	mtype  REDLIGHT:	3\n");
	printf("	mtype  CANWALK:	2\n");
	printf("	mtype  STOP:	1\n");
	printf("	chan plchan (=%d):	len %d:\t", now.plchan, q_len(now.plchan));
	c_chandump(now.plchan);
	printf("	chan vlchan (=%d):	len %d:\t", now.vlchan, q_len(now.vlchan));
	c_chandump(now.vlchan);
	printf("	chan pedchan (=%d):	len %d:\t", now.pedchan, q_len(now.pedchan));
	c_chandump(now.pedchan);
	printf("	bit    push:	%d\n", now.push);
}
void
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (VL):\n", pid);
	printf("	bit    red:	%d\n", ((P1 *)pptr(pid))->red);
		break;
	case 0:
		printf("local vars proc %d (PL):\n", pid);
	printf("	bit    walk:	%d\n", ((P0 *)pptr(pid))->walk);
		break;
	}
}
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("STOP"); return;
	case 2: Printf("CANWALK"); return;
	case 3: Printf("REDLIGHT"); return;
	case 4: Printf("GREENLIGHT"); return;
	case 5: Printf("RB"); return;
